...
/* Not actually a test. We just want tables to be empty again */
START_TEST(test_write_setup)
{
    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;
    int i;

    /* Note: we can't use table_genocide since it doesn't include all tables */
    ...
    char * truncate_sql[] = {
        ...
        "TRUNCATE TABLE nagios_objectes",
    ...
    for (i = 0; i < ARRAY_SIZE(truncate_sql); i++) {
        mysql_query(main_thread_context->conn, truncate_sql[i]);
    }

    /* Just checking one table for now */
    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_contacts");
    ...
    if (tmp_row != NULL) {
        ck_assert_int_eq(strcmp(tmp_row[0], "0"), 0);
    }
}
END_TEST

/* Also not actually a test. Just want to free the bootstrapped memory */
START_TEST(test_write_teardown)
{
    bootstrap_write_teardown();
...
START_TEST(test_write_commands)
{
    ...
    commands[0].id = 1;
    commands[0].name = "command_1";
    commands[0].command_line = "/usr/bin/echo some text";
    commands[0].next = &(commands[1]);

    commands[1].id = 2;
    commands[1].name = "command_2";
    commands[1].command_line = "/usr/bin/echo some text";
    commands[1].next = &(commands[2]);

    commands[2].id = 3;
    commands[2].name = "command_3";
    commands[2].command_line = "/usr/bin/echo some text";
    commands[2].next = NULL;

    ndo_write_commands(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, commands);

    /* Every member of commands is valid, so we should see 3 entries in the database */
    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_commands WHERE command_line = '/usr/bin/echo some text'");
...
START_TEST(test_write_timeperiods)
{

    /* TODO: split into separate tests for write_timeperiods, write_timeperiods_timeranges. 
     * See note on "degenerate organization" in startup_suite().
     */
    ...
    struct timerange full_day = {};
    full_day.range_start = 0;
    full_day.range_end = 86400;
    /* .next is NULL */

    struct timerange work_day = {};
    work_day.range_start = 32400;
    full_day.range_end = 61200;
    
    // TODO: bootstrap_timeperiods
    num_objects.timeperiods = 2;
    timeperiod timeperiods[2] = {};

    timeperiods[0].id = 0;
    timeperiods[0].name = "24x7";
    timeperiods[0].alias = "24 Hours A Day, 7 Days A Week";
    /* timeperiods[0].days = {&full_day, &full_day, &full_day, &full_day, &full_day, &full_day, &full_day}; */
    for (i = 0; i < 7; i += 1) {
        timeperiods[0].days[i] = &full_day;
    }
    /*timeperiods[0].exceptions = {NULL, NULL, NULL, NULL, NULL};*/
    timeperiods[0].exclusions = NULL;
    timeperiods[0].next = &(timeperiods[1]);

    /* workhours contains more varied timeranges than the rest of the default timeperiods */
    timeperiods[1].id = 1;
    timeperiods[1].name = "workhours";
    timeperiods[1].alias = "Normal Work Hours";

    /* timeperiods[1].days = {NULL, &work_day, &work_day, &work_day, &work_day, &work_day, NULL}; */
    for (i = 1; i < 6; i += 1) {
        timeperiods[1].days[i] = &work_day;
    }
    /*timeperiods[1].exceptions = {NULL, NULL, NULL, NULL, NULL};*/
    timeperiods[1].exclusions = NULL;
    timeperiods[1].next = NULL;

    ndo_write_timeperiods(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, timeperiods);

    /* Every member of timeperiods is valid, so we should see 2 entries in nagios_timeperiods */
    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_timeperiods");
    ...
    /* We have a total of 12 non-NULL timeranges across the timeperiods' days, so expect 12 entries in nagios_timeperiods_timeranges. */
    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_timeperiod_timeranges");
    ...
    if (tmp_row != NULL) {
        ck_assert_int_eq(strcmp(tmp_row[0], "12"), 0);
    }
...
START_TEST(test_write_contacts)
{

    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;

    contact *contacts = (contact *) bootstrap_get_contacts();

    ndo_write_contacts(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, contacts);

    /* Every member of timeperiods is valid, so we should see 2 entries in nagios_timeperiods */
    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_contacts WHERE alias LIKE 'c%'");
    ...
    if (tmp_row != NULL) {
        ck_assert_int_eq(strcmp(tmp_row[0], "251"), 0);
    }
...
START_TEST(test_write_contactgroups)
{

    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;

    contactgroup *contactgroups = (contactgroup *) bootstrap_get_contactgroups();

    ndo_write_contactgroups(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, contactgroups);

    /* Every member of timeperiods is valid, so we should see 2 entries in nagios_timeperiods */
    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_contactgroups WHERE alias LIKE 'cg%'");
    ...
    if (tmp_row != NULL) {
        ck_assert_int_eq(strcmp(tmp_row[0], "252"), 0);
    }
...
START_TEST(test_write_hosts)
{
    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;

    host *hosts = (struct host *) bootstrap_get_hosts();

    ndo_write_hosts(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, hosts);

    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_hosts WHERE alias LIKE 'h%'");
    ...
    if (tmp_row != NULL) {
        ck_assert_int_eq(strcmp(tmp_row[0], "254"), 0);
    }
...
START_TEST(test_write_hostgroups)
{
    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;

    hostgroup *hostgroups = (struct hostgroup *) bootstrap_get_hostgroups();

    ndo_write_hostgroups(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, hostgroups);

    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_hostgroups WHERE alias LIKE 'hg%'");
    ...
    if (tmp_row != NULL) {
        ck_assert_int_eq(strcmp(tmp_row[0], "253"), 0);
    }
...
START_TEST(test_write_services)
{
    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;

    service *services = (struct service *) bootstrap_get_services();

    ndo_write_services(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, services);

    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_services WHERE display_name LIKE 's%'");
...
START_TEST(test_write_servicegroups)
{

    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;

    servicegroup *servicegroups = (struct servicegroup *) bootstrap_get_servicegroups();

    ndo_write_servicegroups(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, servicegroups);

    mysql_query(main_thread_context->conn, "SELECT COUNT(*) FROM nagios_servicegroups WHERE alias LIKE 'sg%'");
...
void test_first_row(MYSQL *conn, char *query, int value)
{
    ...
    if (tmp_row != NULL) {
        ck_assert_int_eq(atoi(tmp_row[0]), value);
    }
    mysql_free_result(tmp_result);
}

START_TEST(test_write_scheduling_info)
{

    sched_info scheduling_info = {};

    scheduling_info.total_services = 1;
    scheduling_info.total_scheduled_services = 2;
    scheduling_info.total_hosts = 3;
    scheduling_info.total_scheduled_hosts = 4;
    scheduling_info.average_services_per_host = 5;
    scheduling_info.average_scheduled_services_per_host = 6;
    scheduling_info.service_check_interval_total = 7;
    scheduling_info.host_check_interval_total = 8;
    scheduling_info.average_service_execution_time = 9;
    scheduling_info.average_service_check_interval = 10;
    scheduling_info.average_host_check_interval = 11;
    scheduling_info.average_service_inter_check_delay = 12;
    scheduling_info.average_host_inter_check_delay = 13;
    scheduling_info.service_inter_check_delay = 14;
    scheduling_info.host_inter_check_delay = 15;
    scheduling_info.service_interleave_factor = 16;
    scheduling_info.max_service_check_spread = 17;
    scheduling_info.max_host_check_spread = 18;
    scheduling_info.first_service_check = 19;
    scheduling_info.last_service_check = 20;
    scheduling_info.first_host_check = 21;
    scheduling_info.last_host_check = 22;

    ndo_write_runtime_variables(main_thread_context, scheduling_info);


    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'total_services'", 1);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'total_scheduled_services'", 2);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'total_hosts'", 3);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'total_scheduled_hosts'", 4);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'average_services_per_host'", 5);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'average_scheduled_services_per_host'", 6);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'service_check_interval_total'", 7);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'host_check_interval_total'", 8);
    // average_service_execution_time intentionally left alone
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'average_service_check_interval'", 10);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'average_host_check_interval'", 11);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'average_service_inter_check_delay'", 12);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'average_host_inter_check_delay'", 13);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'service_inter_check_delay'", 14);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'host_inter_check_delay'", 15);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'service_interleave_factor'", 16);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'max_service_check_spread'", 17);
    test_first_row(main_thread_context->conn, "SELECT varvalue FROM nagios_runtimevariables WHERE varname = 'max_host_check_spread'", 18);
    // remaining entries are also not written to DB at this time
}
END_TEST

START_TEST(test_write_hostescalations)
{
    MYSQL_ROW tmp_row;
    MYSQL_RES * tmp_result;
    int object_id;

    hostescalation *hostescalations = (struct hostescalation *) bootstrap_get_hostescalations();

    ndo_write_hostescalations(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, hostescalations);

    object_id = ndo_get_object_id_name1(main_thread_context, TRUE, NDO_OBJECTTYPE_HOST, "Host 0");

    char query[256] = {};
    snprintf(query, 256, "SELECT COUNT(*) FROM nagios_hostescalations WHERE host_object_id = %d", object_id);

    test_first_row(main_thread_context->conn, query, 135);
}
END_TEST

START_TEST(test_write_serviceescalations)
{
    ...
    serviceescalation *serviceescalations = (struct serviceescalation *) bootstrap_get_serviceescalations();

    ndo_write_serviceescalations(main_thread_context, NDO_CONFIG_DUMP_ORIGINAL, serviceescalations);

    object_id = ndo_get_object_id_name2(main_thread_context, TRUE, NDO_OBJECTTYPE_SERVICE, "Host 0", "Service 0");
    char query[256] = {};
    snprintf(query, 256, "SELECT COUNT(*) FROM nagios_serviceescalations WHERE service_object_id = %d", object_id);

    test_first_row(main_thread_context->conn, query, 140);
}
END_TEST
...
{
    /* Note: some of the functions for startup have a sort of degenerate organization,
     * where, for functions step_1(), step_2(), ..., step_n() which have the same level
     * of abstraction, step_1 invokes step_2 before returning, step_2 invokes step_3, etc.
     * 
     * This should be fixed eventually (either inline each of the steps into the step_1 
     * function, or invoke each step sequentially), but for now we'll just note where 
     * it happens, and use one test for each combination of steps.
     * 
     * Check the comment at the top of each test to see which steps are covered. The test
     * will be named for the relevant step_1.
     */

    Suite * suite = NULL;
    TCase * tc_startup = NULL;

    suite = suite_create("startup_suite");

    tc_startup = tcase_create("startup thread functions");

    /* Here's the order in which write functions are called in ndo_write_object_config 
    ...
    write_result = ndo_write_contacts(q_ctx, config_type);
    write_result = ndo_write_contactgroups(q_ctx, config_type);
    write_result = ndo_write_hosts(q_ctx, config_type);
    write_result = ndo_write_hostgroups(q_ctx, config_type);
    write_result = ndo_write_services(q_ctx, config_type);
    write_result = ndo_write_servicegroups(q_ctx, config_type);
    write_result = ndo_write_hostescalations(q_ctx, config_type);
    write_result = ndo_write_serviceescalations(q_ctx, config_type);
    */

    /* Tests TODO for this suite:
    super-basic tests - do things get written to each table
    [X] commands
    [X] timeperiods
    [X] contacts
    [X] contactgroups
    ...
    [X] servicegroups
    [X] hostescalations
    [X] serviceescalations

    extended basic tests - do things get written to the aux tables

    [X] timeperiod_timeranges
    [ ] contact_addresses
    [ ] contact_notificationcommands
    [ ] customvariables (contact)
    [ ] customvariablestatus (contact)
    [ ] contactgroup_members
    [ ] host_parenthosts
    [ ] host_contactgroups
    [ ] host_contacts
    [ ] customvariables (host)
    [ ] customvariablestatus (host)
    [ ] hostgroup_members
    ...
    ?? - Not actually called in program, but functions exist (!)
    [X] excluded with LCOV_EXCL_ comments until we decide to start brokering these.
    [ ] hostdependencies
    [ ] servicedependencies

    Regression testing - Make sure reported bugs are actually fixed, test those which can be tested:
    ...
    */
    ...
    tcase_add_test(tc_startup, test_write_setup);
    
    tcase_add_test(tc_startup, test_write_commands);
    tcase_add_test(tc_startup, test_write_timeperiods);
    tcase_add_test(tc_startup, test_write_contacts);
    tcase_add_test(tc_startup, test_write_contactgroups);
    tcase_add_test(tc_startup, test_write_hosts);
    tcase_add_test(tc_startup, test_write_hostgroups);
    tcase_add_test(tc_startup, test_write_services);
    tcase_add_test(tc_startup, test_write_servicegroups);
    tcase_add_test(tc_startup, test_write_hostescalations);
    tcase_add_test(tc_startup, test_write_serviceescalations);
    tcase_add_test(tc_startup, test_write_scheduling_info);

    tcase_add_test(tc_startup, test_write_teardown);


    suite_add_tcase(suite, tc_startup);
