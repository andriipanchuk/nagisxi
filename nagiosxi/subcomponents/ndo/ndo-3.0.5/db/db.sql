...
CREATE TABLE IF NOT EXISTS `nagios_hosts` (
  ...
  `check_command_args` varchar(1024) NOT NULL default '',
...
# Inserts a single entry into nagios_instances if none exist
INSERT INTO nagios_instances (`instance_name`, `instance_description`)
  SELECT * FROM ( SELECT 'localhost', '' ) AS needs_a_name
  WHERE NOT EXISTS ( SELECT instance_name FROM nagios_instances );
...
CREATE TABLE IF NOT EXISTS `nagios_logentries` (
  ...
  `logentry_data` mediumtext NOT NULL default '',
  ...
  KEY `logentry_data` (`logentry_data`(255))
...
CREATE TABLE IF NOT EXISTS `nagios_objects` (
  ...
  `name1` varchar(1023) NOT NULL default '' COLLATE utf8_bin,
  `name2` varchar(1023) NOT NULL default '' COLLATE utf8_bin,
